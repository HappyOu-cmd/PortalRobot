...
Журнал сохраняет 16 числовых позиций: 14 активных источников (ячейка, робот,
станки 1–3, два магазина, оси X/Y/Z, группа осей, motion manager, point manager и
gripper) и две зарезервированные позиции прежних приводов магазинов. Резерв нужен
...
Матрица магазина фиксирована как 12 × 10. Шаг X/Y и две рабочие высоты меняются
через `ST_MAGAZINE_HMI_COMMAND`: gateway записывает запрошенное значение и
импульс, а `FB_MAGAZINE` проверяет PLC-разрешение до изменения фактической
геометрии. Общий шаг X/Y доступен только при выключенных обоих магазинах;
Z-параметры — при выключенном целевом магазине.
...
`PointId`, координаты и скорость, запрещает выход из папки и символические
ссылки.
