...
export const Заполненный: Story = {
  args: { slots: HMI_SCENARIOS.normal().magazines[0].slots, columns: 10 },
};

export const Пустой: Story = {
  args: { slots: HMI_SCENARIOS.emptyMagazine().magazines[0].slots, columns: 10 },
};

export const Редактирование: Story = {
  args: { slots: HMI_SCENARIOS.normal().magazines[0].slots, columns: 10 },
  render: () => {
    const [slots, setSlots] = useState<SlotType[]>(HMI_SCENARIOS.normal().magazines[0].slots);
