...
export const ALARM_EFFECT_LABELS: Record<PlcAlarmEffect, string> = {
  'global-stop': 'Останов ячейки',
  'robot-stop': 'Останов робота',
  'equipment-stop': 'Останов оборудования',
...
const ALARM_TEXTS = alarmCatalog.alarms as Partial<Record<PlcAlarmSource, string[]>>;
const WARNING_TEXTS = alarmCatalog.warnings as Partial<Record<PlcAlarmSource, Record<number, string>>>;
...
export const formatAlarmJournalMessage = (eventType: string, status: string, eventCode: string | null) => {
  if ((eventType !== 'alarm' && eventType !== 'warning') || !eventCode) return null;
  const [sourceCodeText, localCodeText] = eventCode.split(':');
  const source = ALARM_SOURCES[Number(sourceCodeText)];
  const code = Number(localCodeText);
  if (!source || !Number.isInteger(code) || code < 0) return null;
  const severity: PlcAlarmSeverity = eventType === 'warning' ? 'warning' : 'alarm';
  const reason = severity === 'warning' ? WARNING_TEXTS[source]?.[code] : ALARM_TEXTS[source]?.[code];
  if (!reason?.trim()) return null;
  const label = severity === 'warning' ? 'Предупреждение' : 'Авария';
  const restored = status === 'restored' ? `${label} устранен${severity === 'warning' ? 'о' : 'а'}` : label;
  return `${restored}: ${ALARM_SOURCE_LABELS[source]} — ${reason}`;
};
const mapAlarmEvents = (values: Record<string, unknown>, current: PlcAlarmEvent[]) => {
  ...
  for (let index = 1; index <= 100; index += 1) {
    ...
    const effect = ALARM_EFFECTS[numberValue(values, `${root}.eEffect`, severity === 'warning' ? 3 : 2)]
      ?? (severity === 'warning' ? 'warning' : 'equipment-stop');
