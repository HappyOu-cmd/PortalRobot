...
test('creates, updates, disables and deletes managed users', () => {
  ...
  const operator = store.createUser({ username: 'operator.1', displayName: 'Оператор 1', password: 'strong-pass', role: 'operator' });
  assert.equal(store.login('operator.1', 'strong-pass').user.displayName, 'Оператор 1');
  const updated = store.updateUser(operator.id, { displayName: 'Оператор смены', enabled: false });
